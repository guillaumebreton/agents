...
- `agents start <repo> <branch> --agent=opencode` specifies which coding agent to use (supported: `opencode`, `pi`).
...
- **Coding agents** are code-based abstractions (`internal/coding`). Each implementation (e.g. OpenCode, Pi) registers itself and provides its launch command. New agents are added as code, not config.
...
- [x] Pi coding agent implementation
- [x] Desktop notifications on status transitions (working→idle, *→waiting)
- [x] `--agent` flag
- [x] `--debug` flag
...
- Configurable notification rules (opt-in/out per agent or per transition)
